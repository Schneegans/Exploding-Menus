...
public class Icon : Image {
    ...
    public Icon(string icon_name, int size) {
        var cached = cache.get("%s@%u".printf(icon_name, size));

        if (cached == null) {
            this.load_file_at_size(get_icon_file(icon_name, size), size, size);
            cache.set("%s@%u".printf(icon_name, size), this.surface);
        } else {
            this.surface = cached;
        }
    }
